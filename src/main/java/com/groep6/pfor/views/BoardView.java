...
public class BoardView extends View implements IObserver {
    ...
    private void createPlayerList() {
        ...
        for (int i = 0; i < players.size(); i++) {
            ...
            UIPlayerInfo uiPlayerInfo = new UIPlayerInfo(player.getPlayerRole().getRoleCardColor(), i + 1, player.getUsername(), player.getPlayerRole().getCardName(), player.isCurrentTurn(), player.getActionsRemaining());
            ...
            if (tile instanceof City) {
            	...
            	for (Player player: players) {
            		if (player.getCityPlayerIsCurrentlyLocatedIn().equals(city)) {
	            		gc.setFill(player.getPlayerRole().getRoleCardColor());
